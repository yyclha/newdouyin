...
export function _checkImgUrl(cover) {
  ...
  if (typeof cover === 'string') {
    return cover.startsWith('/') || _validateUrl(cover) ? cover : DEFAULT_IMG
  }

  // 如果 cover 是对象或数组
  if (Array.isArray(cover)) {
    // 优先使用第一个 URL
    return cover.length > 0 && (String(cover[0]?.url_list?.[0] || '').startsWith('/') || _validateUrl(cover[0]?.url_list?.[0]))
      ? cover[0].url_list[0]
      : DEFAULT_IMG
  } else if (typeof cover === 'object' && cover.url_list) {
    return String(cover.url_list[0] || '').startsWith('/') || _validateUrl(cover.url_list[0]) ? cover.url_list[0] : DEFAULT_IMG
  }
