...
func (v *VideoModel) DeleteMyVideo(uid, awemeID int64) bool {
	tx := v.DB.Begin()
	if tx.Error != nil {
		variable.ZapLog.Error("DeleteMyVideo failed to start transaction")
		return false
	}

	var count int64
	checkResult := tx.Raw(`SELECT COUNT(*) FROM tb_videos WHERE aweme_id = ? AND author_user_id = ?`, awemeID, uid).Scan(&count)
	if checkResult.Error != nil || count == 0 {
		tx.Rollback()
		return false
	}

	var source sourceResource
	sourceResult := tx.Raw(`SELECT play_addr, cover FROM tb_source WHERE id = ? LIMIT 1`, awemeID).Scan(&source)
	if sourceResult.Error != nil {
		tx.Rollback()
		variable.ZapLog.Error("DeleteMyVideo query source failed", zap.Error(sourceResult.Error))
		return false
	}

	deleteSQLList := []string{
		`DELETE FROM tb_comments WHERE aweme_id = ?`,
		`DELETE FROM tb_diggs WHERE aweme_id = ?`,
		`DELETE FROM tb_collects WHERE aweme_id = ?`,
		`DELETE FROM tb_shares WHERE aweme_id = ?`,
		`DELETE FROM tb_history WHERE aweme_id = ?`,
		`DELETE FROM tb_source WHERE id = ?`,
		`DELETE FROM tb_statistics WHERE id = ?`,
		`DELETE FROM tb_videos WHERE aweme_id = ? AND author_user_id = ?`,
	}

	for _, sql := range deleteSQLList[:7] {
		if result := tx.Exec(sql, awemeID); result.Error != nil {
			tx.Rollback()
			variable.ZapLog.Error("DeleteMyVideo delete relation failed", zap.Error(result.Error))
			return false
		}
	}

	videoDeleteResult := tx.Exec(deleteSQLList[7], awemeID, uid)
	if videoDeleteResult.Error != nil || videoDeleteResult.RowsAffected == 0 {
		tx.Rollback()
		if videoDeleteResult.Error != nil {
			variable.ZapLog.Error("DeleteMyVideo delete video failed", zap.Error(videoDeleteResult.Error))
		}
		return false
	}

	updateUserResult := tx.Exec(
		`UPDATE tb_users SET aweme_count = GREATEST(COALESCE(aweme_count, 0) - 1, 0) WHERE uid = ?`,
		uid,
	)
	if updateUserResult.Error != nil {
		tx.Rollback()
		variable.ZapLog.Error("DeleteMyVideo update user aweme_count failed", zap.Error(updateUserResult.Error))
		return false
	}

	if err := tx.Commit().Error; err != nil {
		variable.ZapLog.Error("DeleteMyVideo commit failed", zap.Error(err))
		return false
	}

	resourceURLs := append(parseMediaResourceURLs(source.PlayAddr), parseMediaResourceURLs(source.Cover)...)
	for _, resourceURL := range resourceURLs {
		if err := file_storage.DeletePublicResource(resourceURL); err != nil {
			variable.ZapLog.Error("DeleteMyVideo delete storage file failed", zap.Error(err), zap.String("resource_url", resourceURL))
		}
	}
...
func parseMediaResourceURLs(raw json.RawMessage) []string {
	...
	var media mediaResourceURLList
	if err := json.Unmarshal(raw, &media); err != nil {
		return nil
	}

	urls := make([]string, 0, len(media.URLList))
	for _, resourceURL := range media.URLList {
...
func (v *VideoModel) InsertVideoByUID(authorUserId int64, playUrl, videoDesc, coverUrl string, privateStatus int) bool {
	...
	var awemeId int64
	if idResult := tx.Raw(`SELECT LAST_INSERT_ID()`).Scan(&awemeId); idResult.Error != nil || awemeId == 0 {
		if idResult.Error != nil {
			variable.ZapLog.Error("Failed to get inserted aweme_id:", zap.Error(idResult.Error))
		} else {
			variable.ZapLog.Error("Failed to get inserted aweme_id: empty result")
		}
		tx.Rollback()
		return false
	}
